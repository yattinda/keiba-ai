...
X_train = train.drop(['着順','オッズ','人気','上がり','走破時間','通過順'], axis=1).values
y_train = train['着順'].values
X_test = test.drop(['着順','オッズ','人気','上がり','走破時間','通過順'], axis=1).values
y_test = test['着順'].values
...
tabnet_clf.fit(
    ...
    eval_set=[(X_test_cpu, y_test.cpu().numpy())],
    eval_name=['test'],
...
y_pred_train = tabnet_clf.predict_proba(X_train_cpu.values)[:,1]
y_pred = tabnet_clf.predict_proba(X_test_cpu.values)[:,1]
